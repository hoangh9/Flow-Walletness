...
type Service interface {
	...
	Create(ctx context.Context, sync bool , request AccountRequest) (*jobs.Job, *Account, error)
	...
	GetAddressByUsername(username string , password string) (Account, error)
...
func (s *ServiceImpl) Create(ctx context.Context, sync bool , Request AccountRequest) (*jobs.Job, *Account, error) {
	log.WithFields(log.Fields{"sync": sync}).Trace("Create account")

	if !sync {
		attrs := usernameCreateJobAttributes{Request}
		...
		job, err := s.wp.CreateJob(AccountCreateJobType, "",jobs.WithAttributes(attrBytes))
	...
	account, _, err := s.createAccount(ctx,Request)
	if err != nil {
		return nil, nil, err
	}
...
// Username returns a specific account, does not include private keys
func (s *ServiceImpl) GetUsername(username string) (Account, error) {
	log.WithFields(log.Fields{"username": username}).Trace("Account details")

	account, err := s.store.GetUsername(username)
	if err != nil {
		return Account{}, err
	}
...
// Details returns a specific account, does not include private keys
func (s *ServiceImpl) GetAddressByUsername(username string , password string) (Account, error) {
	log.WithFields(log.Fields{"username": username}).Trace("Account details by username")

	account, err := s.store.AccountDetailByUsername(username, password)
	if err != nil {
		return Account{}, err
	}
...
func (s *ServiceImpl) createAccount(ctx context.Context , request AccountRequest) (*Account, string, error) {
