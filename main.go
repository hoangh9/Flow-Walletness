...
func runServer(cfg *configs.Config) {
	...
	rv.Handle("/username/{username}", accountHandler.GetUsername()).Methods(http.MethodGet) // username
	rv.Handle("/getAddressByUsername", accountHandler.GetAddressByUsername()).Methods(http.MethodPost) // Get Address By UserName
